...
class Dataset(Dataset):
    def __init__(self, dir_path: pathlib.Path):
        ...
        assert len(self.wavlist) > 0, "No .wav files found in the directory."
...
def main():
    ...
    if args.mode == "predict_file":
        ...
        with open(args.out_path, "w") as fw:
            fw.write(f"{basename(args.inp_path)},{score[0]}\n")
    else:
        ...
        for batch in tqdm.tqdm(loader):
            fnames_batch, wavs_batch = batch
            scores = scorer.score(wavs_batch.to(device))
            with open(args.out_path, 'a') as fw:
                for fname, s in zip(fnames_batch, scores):
                    fw.write(f"{fname},{s}\n")
